...
std::vector<int> available_vjoy_devices;
int available_vjoy_device_count = 0;
int available_vjoy_device_id = 0;
...
void refresh_available_vjoy_devices() {
	available_vjoy_devices.clear();

	if (!vJoyEnabled()) { 
		available_vjoy_devices.push_back(-2);
		//available_vjoy_devices.push_back("vJoy is not running.");
		return;
	}

	GetNumberExistingVJD(&available_vjoy_device_count);
	if (available_vjoy_device_count == 0) {
		available_vjoy_devices.push_back(-1);
		//available_vjoy_devices.push_back("No vJoy devices enabled.");
		return;
	}

	for (int i = 0; i < 16; ++i) {
		int index = i + 1;
		VjdStat status = GetVJDStatus(index);
		if (status != VJD_STAT_MISS) {
			available_vjoy_devices.push_back(index);
		}
	}
}
...
void imgui_midi_port_manager() {
	...
	if (ImGui::Button("Refresh##midiports") && midi_in->isPortOpen() == false) {
	...
	if (available_midi_port_count > 0) {
		...
		const char* label = midi_port_open ? "Close MIDI Port" : "Open MIDI Port";
...
bool feeding_vjoy = false;
void imgui_vjoy_device_manager() {

	if (ImGui::Button("Refresh##vjoydevices")) {
		refresh_available_vjoy_devices();
	}
	for (int i = 0; i < available_vjoy_device_count; ++i) {
		ImGui::SameLine();

		char vjoy_id_label[8];
		sprintf_s(vjoy_id_label, "%d", available_vjoy_devices[i]);
		ImGui::Button(vjoy_id_label);
	}
	//ImGui::SameLine();

	//const char* label = feeding_vjoy ? "Stop Feeding vJoy Device" : "Start Feeding vJoy Device";
	//if (ImGui::Button(label)) {
	//	int dev_id = available_vjoy_devices[available_vjoy_device_id];
	//	
	//	// if not feeding try to acquire vjoy device
	//	if (!feeding_vjoy) {
	//		feeding_vjoy = AcquireVJD(dev_id);
	//		
	//		if (!feeding_vjoy) {
	//			VjdStat status = GetVJDStatus(dev_id);
	//			imgui_show_error(status, "Failed to acquire vJoy device."); 
	//		}
	//	}
	//	else {
